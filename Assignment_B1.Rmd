...
Test the basic functionality of `tukey2matrix`. Specifically, test for the following:\
1. That the column of the `tukey2matrix()` output matches what we would expect using the `iris` dataset.\
2. That the values in the column of the `tukey2matrix()` output with `significance = FALSE` matches what we would expect using the `iris` dataset.\
3. That the values in the `TukeyHSD()` output are mapped correctly in `tukey2matrix()`.\
4. That calling a specific element from the `TukeyHSD()` output into `tukey2matrix()` actually results in an output dataframe from that element.
...
Additionally, test for common errors. Note that while `tukey2matrix()` exclusively takes `TukeyHSD()` outputs, it should be able to take any variation of `TukeyHSD()` output. Specifically:  
